...
GENERATED_ROOT = ROOT / "tools" / ".trade_doctor_generated"
...
def ensure_generated_stubs() -> None:
    stamp = GENERATED_ROOT / ".stamp.json"
    protos = _proto_files()
    latest_proto_mtime = max(int(p.stat().st_mtime_ns) for p in protos)

    if stamp.exists():
        try:
            cached = json.loads(stamp.read_text())
            if cached.get("latest_proto_mtime_ns") == latest_proto_mtime:
    ...
    args.extend(str(p) for p in protos)
    rc = protoc.main(args)
    if rc != 0:
        raise RuntimeError(f"grpc_tools.protoc failed with exit code {rc}")

    stamp.write_text(json.dumps({"latest_proto_mtime_ns": latest_proto_mtime}, indent=2))
    if str(GENERATED_ROOT) not in sys.path:
        sys.path.insert(0, str(GENERATED_ROOT))
