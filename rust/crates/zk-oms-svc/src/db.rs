...
pub async fn load_config(pool: &PgPool, oms_id: &str) -> Result<RawConfigData, sqlx::Error> {
    ...
    Ok(RawConfigData { oms_config, account_routes, gw_configs, refdata, trading_configs })
}
...
async fn load_oms_config(pool: &PgPool, oms_id: &str) -> Result<OmsConfigEntry, sqlx::Error> {
    ...
    let id_rows = sqlx::query(
        "SELECT account_id FROM cfg.account_binding WHERE oms_id = $1"
    )
    .bind(oms_id)
    .fetch_all(pool)
    .await?;
...
async fn load_account_routes(pool: &PgPool, oms_id: &str) -> Result<Vec<OmsRouteEntry>, sqlx::Error> {
...
async fn load_trading_configs(pool: &PgPool) -> Result<Vec<InstrumentTradingConfig>, sqlx::Error> {
    ...
    Ok(rows
        ...
        .map(|r| InstrumentTradingConfig {
            ...
            publish_balance_on_trade:  r.try_get::<bool, _>("publish_balance_on_trade").unwrap_or(true),
            ...
            max_order_size:            r.try_get::<Option<f64>, _>("max_order_size").unwrap_or(None),
        })
        .collect())
}
