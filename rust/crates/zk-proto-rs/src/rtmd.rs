// @generated
// This file is @generated by prost-build.
#[allow(clippy::derive_partial_eq_without_eq)]
#[derive(Clone, Copy, PartialEq, ::prost::Message)]
pub struct PriceLevel {
    #[prost(double, tag="1")]
    pub price: f64,
    #[prost(double, tag="2")]
    pub qty: f64,
    #[prost(int64, tag="3")]
    pub num_of_orders: i64,
    #[prost(int64, tag="4")]
    pub num_of_pos: i64,
}
#[allow(clippy::derive_partial_eq_without_eq)]
#[derive(Clone, PartialEq, ::prost::Message)]
pub struct Trade {
    #[prost(double, tag="1")]
    pub price: f64,
    #[prost(double, tag="2")]
    pub qty: f64,
    #[prost(int64, tag="3")]
    pub timestamp: i64,
    #[prost(string, tag="4")]
    pub side: ::prost::alloc::string::String,
    #[prost(string, tag="5")]
    pub instrument_code: ::prost::alloc::string::String,
}
#[allow(clippy::derive_partial_eq_without_eq)]
#[derive(Clone, PartialEq, ::prost::Message)]
pub struct TickData {
    #[prost(enumeration="TickUpdateType", tag="1")]
    pub tick_type: i32,
    #[prost(string, tag="2")]
    pub instrument_code: ::prost::alloc::string::String,
    #[prost(string, tag="3")]
    pub exchange: ::prost::alloc::string::String,
    #[prost(int64, tag="4")]
    pub original_timestamp: i64,
    #[prost(int64, tag="5")]
    pub received_timestamp: i64,
    #[prost(double, tag="7")]
    pub volume: f64,
    // other fields: TBA

    /// for trade
    #[prost(double, tag="8")]
    pub latest_trade_price: f64,
    #[prost(double, tag="9")]
    pub latest_trade_qty: f64,
    #[prost(string, tag="10")]
    pub latest_trade_side: ::prost::alloc::string::String,
    /// for orderbook
    #[prost(message, repeated, tag="14")]
    pub buy_price_levels: ::prost::alloc::vec::Vec<PriceLevel>,
    #[prost(message, repeated, tag="15")]
    pub sell_price_levels: ::prost::alloc::vec::Vec<PriceLevel>,
}
#[allow(clippy::derive_partial_eq_without_eq)]
#[derive(Clone, PartialEq, ::prost::Message)]
pub struct Kline {
    /// kline trigger ts
    #[prost(int64, tag="1")]
    pub timestamp: i64,
    /// kline end time
    #[prost(int64, tag="2")]
    pub kline_end_timestamp: i64,
    #[prost(double, tag="3")]
    pub open: f64,
    #[prost(double, tag="4")]
    pub high: f64,
    #[prost(double, tag="5")]
    pub close: f64,
    #[prost(double, tag="6")]
    pub low: f64,
    #[prost(double, tag="7")]
    pub volume: f64,
    #[prost(double, tag="8")]
    pub amount: f64,
    #[prost(enumeration="kline::KlineType", tag="9")]
    pub kline_type: i32,
    #[prost(string, tag="10")]
    pub symbol: ::prost::alloc::string::String,
    #[prost(string, tag="11")]
    pub source: ::prost::alloc::string::String,
}
/// Nested message and enum types in `Kline`.
pub mod kline {
    /// enum type for kline frequency
    #[derive(Clone, Copy, Debug, PartialEq, Eq, Hash, PartialOrd, Ord, ::prost::Enumeration)]
    #[repr(i32)]
    pub enum KlineType {
        Kline1min = 0,
        Kline5min = 1,
        Kline15min = 2,
        Kline30min = 3,
        Kline1hour = 4,
        Kline4hour = 5,
        Kline1day = 6,
    }
    impl KlineType {
        /// String value of the enum field names used in the ProtoBuf definition.
        ///
        /// The values are not transformed in any way and thus are considered stable
        /// (if the ProtoBuf definition does not change) and safe for programmatic use.
        pub fn as_str_name(&self) -> &'static str {
            match self {
                KlineType::Kline1min => "KLINE_1MIN",
                KlineType::Kline5min => "KLINE_5MIN",
                KlineType::Kline15min => "KLINE_15MIN",
                KlineType::Kline30min => "KLINE_30MIN",
                KlineType::Kline1hour => "KLINE_1HOUR",
                KlineType::Kline4hour => "KLINE_4HOUR",
                KlineType::Kline1day => "KLINE_1DAY",
            }
        }
        /// Creates an enum from field names used in the ProtoBuf definition.
        pub fn from_str_name(value: &str) -> ::core::option::Option<Self> {
            match value {
                "KLINE_1MIN" => Some(Self::Kline1min),
                "KLINE_5MIN" => Some(Self::Kline5min),
                "KLINE_15MIN" => Some(Self::Kline15min),
                "KLINE_30MIN" => Some(Self::Kline30min),
                "KLINE_1HOUR" => Some(Self::Kline1hour),
                "KLINE_4HOUR" => Some(Self::Kline4hour),
                "KLINE_1DAY" => Some(Self::Kline1day),
                _ => None,
            }
        }
    }
}
#[allow(clippy::derive_partial_eq_without_eq)]
#[derive(Clone, PartialEq, ::prost::Message)]
pub struct FundingRate {
    #[prost(string, tag="1")]
    pub instrument_code: ::prost::alloc::string::String,
    #[prost(string, tag="2")]
    pub exchange: ::prost::alloc::string::String,
    #[prost(int64, tag="3")]
    pub curr_funding_rate_timestamp: i64,
    #[prost(int64, tag="12")]
    pub observe_timestamp: i64,
    #[prost(int64, tag="15")]
    pub next_payment_timestamp: i64,
    #[prost(double, tag="4")]
    pub funding_rate: f64,
    #[prost(double, tag="6")]
    pub mark_price: f64,
    #[prost(double, tag="7")]
    pub index_price: f64,
    #[prost(double, tag="10")]
    pub next_funding_rate: f64,
    #[prost(int64, tag="13")]
    pub next_funding_rate_timestamp: i64,
    #[prost(string, tag="14")]
    pub original_message: ::prost::alloc::string::String,
}
#[allow(clippy::derive_partial_eq_without_eq)]
#[derive(Clone, PartialEq, ::prost::Message)]
pub struct RealtimeSignal {
    /// map to datastore table name
    #[prost(string, tag="1")]
    pub namespace: ::prost::alloc::string::String,
    #[prost(string, tag="2")]
    pub instrument: ::prost::alloc::string::String,
    #[prost(int64, tag="3")]
    pub timestamp: i64,
    #[prost(map="string, double", tag="4")]
    pub data: ::std::collections::HashMap<::prost::alloc::string::String, f64>,
    #[prost(map="string, int64", tag="6")]
    pub data_int: ::std::collections::HashMap<::prost::alloc::string::String, i64>,
    #[prost(map="string, string", tag="7")]
    pub data_str: ::std::collections::HashMap<::prost::alloc::string::String, ::prost::alloc::string::String>,
    /// can hold json message 
    #[prost(string, tag="5")]
    pub message: ::prost::alloc::string::String,
}
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash, PartialOrd, Ord, ::prost::Enumeration)]
#[repr(i32)]
pub enum TickUpdateType {
    TickTypeUnspecified = 0,
    /// order book
    Ob = 1,
    /// trade
    T = 2,
    /// merged; for some exchanges, the order book and trade are merged into one message
    M = 3,
}
impl TickUpdateType {
    /// String value of the enum field names used in the ProtoBuf definition.
    ///
    /// The values are not transformed in any way and thus are considered stable
    /// (if the ProtoBuf definition does not change) and safe for programmatic use.
    pub fn as_str_name(&self) -> &'static str {
        match self {
            TickUpdateType::TickTypeUnspecified => "TICK_TYPE_UNSPECIFIED",
            TickUpdateType::Ob => "OB",
            TickUpdateType::T => "T",
            TickUpdateType::M => "M",
        }
    }
    /// Creates an enum from field names used in the ProtoBuf definition.
    pub fn from_str_name(value: &str) -> ::core::option::Option<Self> {
        match value {
            "TICK_TYPE_UNSPECIFIED" => Some(Self::TickTypeUnspecified),
            "OB" => Some(Self::Ob),
            "T" => Some(Self::T),
            "M" => Some(Self::M),
            _ => None,
        }
    }
}
// @@protoc_insertion_point(module)
