...
def main() -> int:
    ...
    # Single source of truth: the versioned tree (protos/zk/**/v1/*.proto). The module
    # path is derived from the proto `package` (e.g. zk.rtmd.v1 -> zk/rtmd/v1/__init__.py).
    # Multiple protos can share one package (e.g. oms.proto + oms_service.proto both
    # declare zk.oms.v1), so merge their specs per package before writing a single
    # compat block — otherwise a later proto would overwrite an earlier one's aliases.
    merged: dict[str, ProtoSpec] = {}
    for proto_path in sorted((proto_dir / "zk").rglob("*.proto")):
        spec = parse_proto(proto_path)
        existing = merged.get(spec.package)
        if existing is None:
            merged[spec.package] = spec
        else:
            existing.type_names.extend(spec.type_names)
            existing.enums.extend(spec.enums)

    updated = 0
    for package, spec in merged.items():
        module_path = out_root / package.replace(".", "/") / "__init__.py"
        if not module_path.exists():
            continue
        alias_lines = build_alias_lines(spec, module_path)
