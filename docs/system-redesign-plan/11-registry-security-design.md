...
**Vault scope**: Vault is used only by Trading Gateways to fetch exchange credentials (API key, API secret, private key). See [09-ops-design.md](09-ops-design.md) §1.
...
Bootstrap tokens (for Pilot registration, see [12-topology-token-registration-design.md](12-topology-token-registration-design.md)) are also provided via env/args, not Vault.
