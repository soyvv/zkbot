...
Value: protobuf-encoded `zk.discovery.v1.ServiceRegistration` (see [04-proto-design.md](04-proto-design.md)).
...
All command responses use `CommandAck` (see [04-proto-design.md](04-proto-design.md)).
...
| `zk.oms.<oms_id>.balance_update.<asset>` | `oms.BalanceUpdateEvent` | per-asset (e.g. `USDT`, `BTC`); use `*` wildcard to subscribe all |
