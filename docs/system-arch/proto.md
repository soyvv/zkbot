...
Used as the value payload in NATS KV entries (see [api_contracts.md](api_contracts.md)).
...
Add a query service in the same package for current snapshots and bounded recent history:

```proto
service RtmdService {
  rpc QueryCurrentTick(QueryCurrentTickRequest) returns (QueryCurrentTickResponse);
  rpc QueryCurrentOrderBook(QueryCurrentOrderBookRequest) returns (QueryCurrentOrderBookResponse);
  rpc QueryCurrentFundingRate(QueryCurrentFundingRateRequest) returns (QueryCurrentFundingRateResponse);
...
- requests should use ZK symbol conventions (`instrument_id`) as the primary identifier
- responses reuse `zk.rtmd.v1` payload types where possible
- `QueryKlines` is for bounded recent history, not arbitrary deep archival retrieval
