...
**Implemented — NATS-based latency metrics (`zk-oms-svc`):**

End-to-end order latency segments published as `LatencyMetricBatch` to `zk.oms.<oms_id>.metrics.latency`:

| Segment | Formula | Description |
|---|---|---|
| `oms_through` | `t3 - t1` | Internal OMS queue wait + core processing |
| `gw_send` | `t3r - t3` | GW gRPC send overhead |
| `gw_processing` | `t5 - t4` | Time from GW received to BOOKED published |
| `nats_delivery` | `t6 - t5` | NATS delivery from GW publish to OMS recv |
| `oms_update_pub` | `t7 - t6` | OMS processing NATS report and publishing update |
| `client_recv` | `t8 - t7` | NATS delivery from OMS publish to client recv |

Timestamps: `t0`(client), `t1`(OMS gRPC handler entry), `t3`/`t3r`(before/after GW send), `t4`(GW entry, `gw_received_at_ns`), `t5`(GW BOOKED pub), `t6`(OMS NATS recv), `t7`(OMS update pub), `t8`(client recv).

Note: `t1 = 0` for `BatchPlaceOrders` — `oms_through` segment unavailable for batch.
...
RTMD Gateway:
- `rtmd_upstream_reconnect_total` counter
- `rtmd_publish_drop_total` counter (for non-durable tick/orderbook fanout)
- `rtmd_interest_lease_count` gauge
- `rtmd_effective_stream_count` gauge
- `rtmd_query_latency_ms` histogram
- `rtmd_query_error_total` counter
...
- [ ] define the RTMD gateway health model in more detail: upstream disconnected, degraded query path, partial stream coverage, publish lag
- [ ] define how RTMD health is surfaced to Pilot and operators
