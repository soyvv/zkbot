...
| Redis | fast cache + non-critical read replica: OMS open orders, recent closed orders, positions, counters |
| NATS KV | live service registry and ephemeral RTMD subscription leases |
...
## 1.1 RTMD storage and transport decisions

RTMD is intentionally split by channel family.
...
- `tick` and `orderbook`
  ...
  - slow-consumer loss is acceptable
- `kline`
  - JetStream-backed stream
  - suitable for replay/recovery and bounded recent-history workflows

Query semantics:

- the RTMD gateway is not the semantic source of truth for current or historical market data
- RTMD query APIs should be defined in terms of venue-backed data retrieval
- gateway-local transient runtime state does not constitute an authoritative cache contract
...
- [ ] define whether any persisted kline retention beyond JetStream is needed and where it should live
- [ ] align refdata-change handling with the refdata service design
- [ ] define RTMD health and lag metrics publication in the ops design
- [ ] decide whether any additional RTMD persistence belongs in Recorder rather than the gateway
