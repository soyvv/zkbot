...
| `zk-gw-svc` | **planned** | binary (per venue) — Trading Gateway gRPC service host |
| `zk-collocated-svc` | **planned** | binary — 2-layer or all-in-one host (engine + OMS ± GW plugin in one process, inter-thread channels) |
...
Note: `zk-oms-svc` currently does not depend on `zk-trading-sdk-rs` (planned SDK); it manages GW gRPC clients directly in `gw_client.rs`.
...
See [sdk.md](sdk.md) for full API design.
...
**Implemented** at `zkbot/rust/crates/zk-oms-svc/`.

Source modules:
- `config.rs` — `OmsSvcConfig` (all fields from `ZK_*` env vars via `envy`)
- `main.rs` — full startup sequence
- `grpc_handler.rs` — tonic `OmsService` implementation
- `oms_actor.rs` — single-writer actor (`OmsCommand` channel, `process_command`, `spawn_writer`)
- `nats_handler.rs` — NATS subscribers for gateway reports; `NatsPublisher`
- `redis_writer.rs` — Redis persistence (orders, balances, positions)
- `gw_client.rs` — `GwClientPool` (tonic gRPC clients to gateways)
- `db.rs` — PostgreSQL config loading
- `latency.rs` — `LatencyTracker`, `TimestampRecord`, `publish_latency_batch`

Startup sequence (implemented in `main.rs`):
1. Connect NATS, Redis, PostgreSQL
2. Load `ConfdataManager` from PostgreSQL
3. Warm-start OMS state from Redis
4. Init `OmsCore`
5. Watch gateway KV (`svc.gw.*`) and build `GwClientPool`
6. Start OMS writer actor task
7. Start gRPC server
8. Register `svc.oms.<oms_id>` in NATS KV
9. Start NATS subscribers (gateway reports → actor channel)
10. Start cleanup timer

Deferred / stubs:
- Step 7 of planned sequence: `QueryAccountBalance` on startup — balance reconciliation is stubbed; TODO comment in `main.rs`
- `order_resync_interval_secs` (default 60s) and `balance_resync_interval_secs` (default 60s) config fields exist but periodic resync tasks are not spawned yet
- `OmsCore::calc_balance_changes_for_report` returns `None` (see `zk-oms-rs` stubs)

Config env vars (via `ZK_` prefix):
- `ZK_OMS_ID`, `ZK_NATS_URL`, `ZK_REDIS_URL`, `ZK_DATABASE_URL`
- `ZK_GRPC_PORT`, `ZK_GRPC_HOST`
- `ZK_METRICS_INTERVAL_SECS` (default 10), `ZK_METRICS_MAX_PENDING` (5000), `ZK_METRICS_MAX_COMPLETE` (10000)
- `ZK_ORDER_RESYNC_INTERVAL_SECS` (default 60), `ZK_BALANCE_RESYNC_INTERVAL_SECS` (default 60), `ZK_CLEANUP_INTERVAL_SECS` (default 600)

### `zk-mock-gw`

**Implemented** at `zkbot/rust/crates/zk-mock-gw/`.

Implements both legacy `ExchangeGatewayService` and new `GatewayService` (`zk.gateway.v1`).
Used for dev/testing. Provides auto-fill simulation and publishes execution reports to NATS.
Source modules: `gateway.rs`, `zk_gateway.rs`, `fill.rs`, `state.rs`.
